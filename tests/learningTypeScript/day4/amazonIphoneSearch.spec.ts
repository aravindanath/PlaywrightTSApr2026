...
test('Search for iPhone 17 Pro Max deal on Amazon India', async ({ page }) => {
    ...
    await searchBox.fill('iPhone 17 Pro Max deal');
    ...
    await expect(page).toHaveURL(/iPhone\+17\+Pro\+Max\+deal|iPhone%2017%20Pro%20Max%20deal/i);
    ...
    await expect(resultsHeader).toContainText('iPhone 17 Pro Max deal');

    // Take a full page screenshot of the search results
    await page.screenshot({ path: 'screenshots/amazon_iphone17_promax_deal.png', fullPage: true });

    console.log('Amazon search for iPhone 17 Pro Max deal completed successfully.');
});
